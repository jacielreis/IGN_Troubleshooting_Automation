...
net_connect = ConnectHandler(
    device_type='cisco_ios_telnet', ip=ipaddr_tun,
    username=user_passw.user, password=user_passw.passw)
...
if dev_vers in 'C8':
    net_connect = ConnectHandler(
        device_type='cisco_ios_telnet', ip='192.168.5.142',
        username='ign', password='auto')
...
if dev_vers in 'C8':
    net_connect = ConnectHandler(device_type='cisco_ios_telnet', ip='192.168.5.142', username='ign', password='auto')
    inet_counters = net_connect.send_command(
    ...
    net_connect = ConnectHandler(
        ...
        username='ign', password='auto')
    inet_counters = net_connect.send_command(
        'show interfaces fastethernet0/1 | i duplex|CRC|collision|Last clearing|Description')
    net_connect.disconnect()
